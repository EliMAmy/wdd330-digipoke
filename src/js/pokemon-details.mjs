...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (id < 1 || id > MAX_POKEMONS) {
        return (window.location.href = "digipoke/pokemon.html");
    }
    currentPokemonId = id;
    loadPokemon(id);
});
...
async function loadPokemon(id) {
    try { 
        ...
        if (currentPokemonId === id) {
            ...
            const flavorText = getEnnglishFlavorText(pokemonSpecies);
            ...
            window.history.pushState({}, "", `digipoke/details.html?id=${id}`)
        }
        return true;
    }
    catch (error) {
        console.error("n error occured while fetching Pokemon Data:", error);
        return false;
    }
...
function getEnnglishFlavorText(pokemonSpecies) {
